...
function TaskItem({ task, onToggleComplete, onEdit, onDelete, onSelectTask }) {
  ...
  const fetchSubtasks = async () => {
    try {
      const response = await fetch(`http://localhost:8000/tasks/${task.id}/subtasks`);
  ...
  const handleGenerateSubtasks = async (e) => {
    ...
    try {
      ...
      const newSubtasks = await response.json();
...
export default function Home() {
  ...
  const fetchTasks = async () => {
    try {
      ...
      setTasks(data.filter(task => !task.parent_id));
      setError(null);
    } catch (error) {
